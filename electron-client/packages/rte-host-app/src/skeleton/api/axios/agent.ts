...
export const uploadAgent = axios.create({
  baseURL: HOST,
  timeout: 15 * 1000,
});
...
export const notice: Notice = (type, title, content) => {
  notification[type]({
    message: title,
    description: content,
  });
...
//     if (config.params) {
//       config.params = transformPropertyName(config.params, key =>
//         key.replace(/[A-Z]/g, str => '_' + str.toLowerCase())
//       );
//     }
